    ...
    for _ in range(steps_per_sample):
        loss, L1, L3, _ = sess.run([loss_tnsr, L1_tnsr, L3_tnsr, optimizer],
                                   feed_dict={t_interior_tnsr: t_interior, X_interior_tnsr: X_interior,
                                              t_terminal_tnsr: t_terminal, X_terminal_tnsr: X_terminal})
...
for i, curr_t in enumerate(valueTimes):
    ...
    # compute normalized density at all x values to plot and current t value
    t_plot = curr_t * np.ones_like(X_plot.reshape(-1, 1))
    fitted_optimal_control = sess.run([numerical_optimal_control],
                                      feed_dict={t_interior_tnsr: t_plot, X_interior_tnsr: X_plot.reshape(-1, 1)})
...
fitted_optimal_control = sess.run([numerical_optimal_control],
                                  feed_dict={t_interior_tnsr: t_plot, X_interior_tnsr: X_plot})
